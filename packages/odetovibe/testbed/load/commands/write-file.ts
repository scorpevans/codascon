...
/**
 * Deep-merge an interface declaration.
 *
 * - `extends`, `typeParameters` → codegen owns; always updated.
 * - Properties and method signatures → codegen owns; always updated.
 * - Members present in existing but absent from generated → preserved.
 * - JSDoc (interface + all members) → user owns; always preserved.
 */
function mergeInterface(existing: InterfaceDeclaration, generated: InterfaceDeclaration): void {
  const genStruct = generated.getStructure();
  existing.set({
    typeParameters: genStruct.typeParameters,
    extends: genStruct.extends,
    docs: existing.getJsDocs().map((d) => d.getStructure()),
  });
  ...
  for (const genMethod of generated.getMethods()) {
    ...
    if (existingMethod) {
      existingMethod.set({
        ...genMethod.getStructure(),
        docs: existingMethod.getJsDocs().map((d) => d.getStructure()),
      });
    } else {
      existing.addMethod(genMethod.getStructure());
    }
  }
}
